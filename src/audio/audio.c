...
void generate_sample() {
	pins.pins.out = 0;
	pins.pins.out += lowpass(lowpass(render_oscillator(0), 0, 0), 3, 0);
	pins.pins.out += lowpass(lowpass(render_oscillator(1), 1, 1), 4, 1);
	pins.pins.out += lowpass(lowpass(render_oscillator(2), 2, 2), 5, 2);
}
...
float lowpass(float sample, uint8_t n, uint8_t osc) {
	...
	cutoff = oscillators[osc].oscillator.low / 256.0 * (20000.0 / SAMPLE_RATE);
	c2 = cutoff * cutoff;
